...
type (
	Opts struct {
		...
		Scrape struct {
			...
			TimeKubernetesService *time.Duration `long:"scrape-time-kubernetesservice"  env:"SCRAPE_TIME_KUBERNETESSERVICE"    description:"Scrape time for Kubernetes Service metrics (time.duration)"`
		...
		Portscan struct {
			...
			Prallel   int           `long:"portscan-parallel"             env:"PORTSCAN_PARALLEL"                        description:"Portscan parallel scans (parallel * threads = concurrent gofuncs)"  default:"2"`
